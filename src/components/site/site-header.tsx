...
import { BeeFlight } from "@/components/fx/bee-flight";
...
export function SiteHeader() {
  return (
    <>
      {/* Fixed and click-through, so it belongs to no particular section. */}
      <BeeFlight />
          ...
          <nav className="mx-auto flex max-w-6xl flex-col gap-1 px-4 py-3 text-[15px] font-medium">
            {LINKS.map((l) => (
              <Link
                key={l.href}
                href={l.href}
                className="py-2 text-text-2 hover:text-text"
              >
                {l.label}
              </Link>
            ))}
