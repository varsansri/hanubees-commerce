...
export const metadata: Metadata = {
  title: "Hanubees Commerce — sell the things you make",
  description:
    "A storefront and a real admin, running the same evening you set it up. Hanubees Commerce is the commerce platform built by Hanubees Technologies.",
};
      ...
      <section className="mx-auto w-full max-w-6xl px-4 pt-12 sm:pt-16">
        <div className="grid items-center gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,30rem)] lg:gap-12">
          <div>
            <Link
              href="/"
              className="iso-block-sm rise inline-flex items-center bg-iso-sky px-3 py-1 text-[13px] font-bold tracking-tight text-iso-black"
            >
              A product by Hanubees Technologies
            </Link>
          ...
          {stores.map((s) => (
            <li key={s.id}>
              <Link href={`/store/${s.handle}`} className="group block">
                <div
                  className="iso-block aspect-[3/2] transition-transform duration-200 ease-[var(--ease-out)] group-hover:-translate-y-1"
